...
def main() -> None:
    """Run HTTP server."""
    print("\nPiper-TTS v1.3.0 web server starting.\n")

    voices_dir = Path(sys.argv[0]).parent.joinpath('voices')
    model_default = voices_dir.joinpath('en_US-amy-low.onnx')
...
#        required=True,
    ...
    parser.add_argument(
        ...
        help="Data directory to check for downloaded models (default: ./voices)",
    )
    parser.add_argument(
        "--download-dir",
        "--download_dir",
        default=str(voices_dir),
        help="Path to download voices (default: ./voices)",
    )
    #
    parser.add_argument(
    ...
    print("Voice models folder: " + str(voices_dir))
    print("Default model: " + str(model_path))
    print("Download folder: " + str(download_dir))
    ...
    print("Piper-TTS running on " + hostname + " listening on " + args.host)
    print("This is your endpoint TTS connector http://" + IPAddr + ":" + str(args.port) + "\n")
